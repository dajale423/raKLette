...
class TSVDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.chunksize == 0:
            x = pd.read_csv(self.path, sep = "\t", names=self.header, skiprows=1)
        else:
...
def run_raklette_cov(loader, n_covs, num_epochs, neutral_sfs_filename, output_filename, lr, gamma, cov_sigma_prior = torch.tensor(0.1, dtype=torch.float32), mu_col = 0, bin_col = 1, cov_col = 2, print_loss = True):
    #lr is initial learning rate
